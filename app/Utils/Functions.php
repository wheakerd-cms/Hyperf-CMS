...
final class Functions
{
	 ...
	 * @return array
	 */
	public static function extractOne(array $list, string $key, mixed $value): array
	{
		foreach ($list as $item) {
	...
	/**
	 * 从二维数组中提取符合条件的列值，返回索引数组
	 *
	 * @return string[]|integer[]|float[]
	 */
	public static function extraColumn(
		array  &$list,
		string $primaryKey,
		string $parentKey,
		string $column,
	...
	): array
	{
		...
		foreach ($list as $item) {
			...
			$data [] = empty($children) ? $value : array_merge($value, $children);
